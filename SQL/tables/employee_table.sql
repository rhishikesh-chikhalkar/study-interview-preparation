-- Create the employee table to store employee information and their
-- department associations
CREATE TABLE IF NOT EXISTS
interview_prep.employee_table (
    -- Unique auto-incrementing identifier for each employee
    employee_id SERIAL PRIMARY KEY,
    employee_name VARCHAR(50),      -- Employee's name
    employee_salary INT,            -- Employee's salary
    -- Foreign key referencing dept_table(dept_id)
    dept_id INT REFERENCES interview_prep.dept_table (dept_id)
);

-- Seed the employee table with sample data across multiple departments
-- This allows testing of queries like finding Nth highest salaries per
-- department
